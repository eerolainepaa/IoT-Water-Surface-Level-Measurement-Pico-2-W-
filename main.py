...
def write_config(o):
	# Save new credentials and restart
	try:
	...
	for part in parts[1:]:
		if len(part) >= 2:
			try:
				...
				decoded += '%' + part # If not hex, don't change
		else:
			decoded += '%' + part
...
# --- Sensor reading ---
def val2temp(v):
	return round((((v / 65535) * 3.3) - V0C) / TC, 1)
...
def read_data():
	...
	temperature = val2temp(t)

	lcd_write(height,temperature)
...
# --- LCD ---
def lcd_write(height,temperature):
	lcd.clear()
	lcd.move_to(0, 0)
	lcd.putstr(f"Height: {height:6.1f}cm")
	lcd.move_to(0, 1)
	lcd.putstr(f"C: {temperature:6.1f}C")
